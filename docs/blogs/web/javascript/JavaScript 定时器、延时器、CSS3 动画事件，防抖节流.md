...
// 小球停止运动条件
if ((currentLocation >= target && step > 0) || (currentLocation <= target && step < 0)) {
  currentLocation = target
  clearInterval(el.timer)
}
```
...
.red-packet { width: 120px; height: 168px; background: url("./images/redPacket.png") no-repeat;
...
/* 中奖显示效果 */ .show-red-packet { width: 400px; height: 579px; background-image:
url("./images/redPacket2.png"); background-size: cover; position: absolute; top: 50%; left: 50%;
transform: translate(-50%, -50%); z-index: 3; } /* 关闭按扭 */ .show-red-packet .close { display:
block; content: ""; width: 50px; height: 50px; position: absolute; top: 0; right: -50px;
background-color: #fff; border-radius: 50%; cursor: pointer; color: #333; text-align: center;
line-height: 50px; font-size: 30px; } /* 中奖后，显示金额样式 */ .show-red-packet .money { width:
100%; height: 50px; font-size: 50px; color: #fff; text-align: center; position: absolute; bottom:
150px; display: block; user-select: none; }
```
...
```js
var bgColor = ['pink', 'skyblue', 'turquoise', 'khaki', 'salmon', 'thistle', 'orange', 'red']
...
<div class="login" id="J_login">
  ...
  <div class="login-row">
    <button type="submit" name="login" class="button-login" id="button-login">登录</button>
  </div>
</div>
```
...
<div class="magnifier-preview">
  ...
  <ul class="small-img" id="J_small-img-wrap">
    <li>
      <img src="./images/small1.jpg" width="75" class="active" data-src="./images/big1.jpg" />
    </li>
    <li>
