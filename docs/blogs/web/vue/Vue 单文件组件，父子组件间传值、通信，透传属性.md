...
<body>
  <div data-v-7a7a37b1 class="box" style="--7a7a37b1-color: red;">Hello Vue3</div>
</body>
```
...
<template>
  ...
  <div>用户年龄：{{ age }} 岁</div>
  <div>
    用户地址： {{ typeof address === "string" ? address : address.province + address.city }}
  </div>
  <div>---------------------------------</div>
</template>
```
...
<template>
  ...
  <!--以上方式等价于-->
  <Person :username="userInfo.username" :age="userInfo.age" :tel="userInfo.tel" />
</template>
```
...
```html
<List @update-event="update" />
methods: { update(a, b, c) { // a b c对应传过来的数据 5 6 7 console.log(a, b, c); } //
或通过arguments来接收 /* update() { Array.from(arguments).forEach((item) => { this.arr.push(item) })
} */ }
```
...
<template>
  <List :list="list" @add-event="(...arg) => list.push(...arg)" @del-event="del" />
</template>
```
...
<template>
  <div class="popup" v-show="bool">
    <textarea :value="title" @input="$emit('update:title', $event.target.value)"></textarea>
    <div class="close" @click="$emit('update:bool', false)">确认</div>
  </div>
  <div class="mask" v-show="bool"></div>
</template>
...
<div class="list" style="{font-size:'30px'}" title="组件自身属性">List组件，是一个单根组件</div>
```

> 以上代码最终渲染效果如下

```html
<div class="list active" style="color: red;" title="透传属性">List组件，是一个单根组件</div>
```
