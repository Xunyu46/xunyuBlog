...
```html
<router-link to="/news/tab1" exact-active-class="exact-active">最新动态</router-link>
```
...
```html
<!--以下写法是错的-->
<router-link :to="{name:'home',path:'/'}" active-class="active">Home</router-link>
...
<script setup>
  ...
  axios
    .get(`https://www.fastmock.site/mock/6ec78e345df340241e1f5043f0167833/icode/detail/${id}`)
    .then((res) => {
...
<template>
  ...
  <van-tabbar active-color="#ee0a24" v-model="active">
    ...
    <van-tabbar-item icon="shopping-cart-o" to="/mycart">购物车</van-tabbar-item>
