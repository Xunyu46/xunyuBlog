...
```html
<AnchoredHeading :level="3" href="http://www.icodingedu.com">艾编程</AnchoredHeading>
...
<script>
  ...
  export default {
    props: ['level', 'href'],
    setup(props, { slots }) {
      return () => h('h' + props.level, h('a', { href: props.href }, slots.default()))
    },
  }
</script>
```
...
<script>
  ...
  export default {
    setup() {
      const foo = ref('foo')
      const bar = ref(true)
      return () => h('div', { class: [foo.value, { bar }], style: { color: 'red' } })
    },
  }
</script>
...
<script>
  ...
  export default {
    setup() {
      ...
      return () =>
        h('div', [
          h('button', { onClick: () => (isShow.value = !isShow.value) }, '切换'),
...
export default function MyComponent(props, { slots, emit, attrs }) {
  ...
  return h('div', { class: 'box' }, [
    h('button', { onClick: () => emit('update', 'xxxx函数式组件xxxx') }, '更新'),
    h('div', props.title),
    slots.default(), // 默认插槽
    slots.main(), // 具名插槽
  ])
...
<template>
  <div class="menu">
    <template v-for="item in menuList">
      <!--没有子菜单，就渲染一级-->
      <menuitem v-if="!item.children" :key="item.id" :href="item.href">{{ item.title }}</menuitem>
      <!--有子菜单，就渲染二级-->
...
<template>
  <div class="sub-menu">
    ...
    <div class="sub-item">
      <menuitem v-for="child in data.children" :key="child.id" :href="child.href">
        {{ child.title }}
      </menuitem>
    </div>
  </div>
  ...
  <SubMenu>
    ...
    <template #default>
      <template v-for="child in data.children" :key="child.id">
        <menuitem v-if="!child.children" :href="child.href">{{ child.title }}</menuitem>
        <!-- 递归 -->
