import{_ as p,r as e,o,c,b as s,d as n,e as l,a}from"./app-48690364.js";const i={},u=a(`<h1 id="js-面向对象、分层设计、动态数据遍历最佳实践与应用" tabindex="-1"><a class="header-anchor" href="#js-面向对象、分层设计、动态数据遍历最佳实践与应用" aria-hidden="true">#</a> JS 面向对象、分层设计、动态数据遍历最佳实践与应用</h1><p>本节内容开始使用上一节学到的 async/await 机制来实现同步代码的编写风格，调用异步请求的代码可以很好为我们整个项目的网络请求提供很好的支持。</p><p>接下来我们就开始应用这些知识来实现动态接口请求数据的方式，完成真实项目开发中 JS 面向对象、分层设计、动态数据遍历的最佳实践与应用。</p><ul><li>分类列表选项的动态化</li><li>课程预览组件封装 与 课程列表展示</li><li>上拉触底、下拉刷新，加载更多数据</li><li>内容标签页、分类切换数据联动</li></ul><h2 id="一、分类列表选项的动态化" tabindex="-1"><a class="header-anchor" href="#一、分类列表选项的动态化" aria-hidden="true">#</a> 一、分类列表选项的动态化</h2><p>运用封装好的 <code>request</code> 网络请求库、async/await 机制、分层设计实现分类列表数据的动态化。</p><h3 id="_1、创建课程分类列表接口-mock-数据" tabindex="-1"><a class="header-anchor" href="#_1、创建课程分类列表接口-mock-数据" aria-hidden="true">#</a> 1、创建课程分类列表接口 Mock 数据</h3><p>在 fastmock 中创建课程分类列表数据接口 <code>/api/course/category</code> 请求方式为 GET</p><div class="language-json line-numbers-mode" data-ext="json"><pre class="language-json"><code><span class="token punctuation">{</span>
...
<span class="token function">Page</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span><span class="token punctuation">,</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>总结</p><p>我们发现刚刚在模型层中，重新定义一个插入全部选项的方法后，就可以起到很灵活的方法复用了。</p><p>我们是在定义的 <code>getCategoryListWithAll()</code> 方法中调用了原来的 <code>getCategoryList()</code> 获取分类数据列表的方法来实现数据定制，在页面逻辑中只需要修改调用的方法即可实现数据的切换。</p><blockquote><p>这就是我们前面说到的：分离调用 与 内部实现，实现功能解耦的具体实现了。</p></blockquote><h2 id="二、课程预览组件封装-与-课程列表展示" tabindex="-1"><a class="header-anchor" href="#二、课程预览组件封装-与-课程列表展示" aria-hidden="true">#</a> 二、课程预览组件封装 与 课程列表展示</h2>`,32),r={href:"http://localhost:8888/vip/applet/custom-component-practice.html#%E5%85%AD%E3%80%81%E8%87%AA%E5%AE%9A%E4%B9%89%E7%BB%84%E4%BB%B6%E5%B0%81%E8%A3%85-%E6%9C%80%E4%BD%B3%E5%AE%9E%E8%B7%B5%E6%80%BB%E7%BB%93",target:"_blank",rel:"noopener noreferrer"},k=a(`<blockquote><p>具体步骤如下</p></blockquote><h3 id="_1、创建课程列表接口-mock-数据" tabindex="-1"><a class="header-anchor" href="#_1、创建课程列表接口-mock-数据" aria-hidden="true">#</a> 1、创建课程列表接口 Mock 数据</h3><p>在 fastmock 中创建课程列表数据接口 <code>/api/course/list</code> 请求方式为 GET</p><div class="language-json line-numbers-mode" data-ext="json"><pre class="language-json"><code><span class="token punctuation">{</span>
  <span class="token property">&quot;code&quot;</span><span class="token operator">:</span> <span class="token string">&quot;0000&quot;</span><span class="token punctuation">,</span>
  <span class="token property">&quot;data&quot;</span><span class="token operator">:</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>实现效果</p><p><img src="https://www.arryblog.com/assets/img/image-20230427024936814.827f2ffe.png" alt="image-20230427024936814"></p><h2 id="三、上拉触底、下拉刷新-加载更多数据" tabindex="-1"><a class="header-anchor" href="#三、上拉触底、下拉刷新-加载更多数据" aria-hidden="true">#</a> 三、上拉触底、下拉刷新，加载更多数据</h2><p>上拉触底 和 下拉刷新加载更多数据在移动端 和 小程序项目开发中是非常常见的功能需求，也是必备技能。同时，深入 JavaScript 面向对象的原理和机制在项目工程中的实践。</p><h3 id="_1、在页面配置文件中开启下拉刷新" tabindex="-1"><a class="header-anchor" href="#_1、在页面配置文件中开启下拉刷新" aria-hidden="true">#</a> 1、在页面配置文件中开启下拉刷新</h3><p>在 <code>/pages/index/index.json</code> 配置文件中，开启下拉刷新并配置样式</p><div class="language-json line-numbers-mode" data-ext="json"><pre class="language-json"><code><span class="token punctuation">{</span>
...
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    ...
    <span class="token property">&quot;desc&quot;</span><span class="token operator">:</span> <span class="token string">&quot;成功&quot;</span>
  <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_4、上拉触底加载下一页的数据" tabindex="-1"><a class="header-anchor" href="#_4、上拉触底加载下一页的数据" aria-hidden="true">#</a> 4、上拉触底加载下一页的数据</h3><p>实现原理：每一次下拉触底都需要获取下一页的数据并且与当前页的数据合并</p><p><strong>实现思路</strong></p><ul><li>①、直接调用模型类中获取课程列表的方法 <code>getCourseList(page,count)</code> 每次下拉刷新改变参数 page 当前页 和 count 每页数量 即可，通过这种方式可以实现，但不优雅 ！</li></ul><blockquote><p>我们之前学过代码的分层设计目的就是：分离调用 与 内部实现，实现功能解耦。</p><p>我们要讲究一个单一原则，当前课程模型它的功能就是为我们提供数据的，页面去调用这个模型时它不应该还需要去维护类似这种 page 和 count 的数据。都应该是模型内在去维护和管理的，它可以暴露一些方法让我们去定制化、去修改它。正常情况下在调用时不应该由页面去关心这些东西。</p><p>否则，每一个使用到课程列表的页面都要去维护一份 page 和 count 包括处理它的数据合并。其实，都应该是模型帮我们做的事情才对。</p></blockquote><ul><li>②、在模型中实现获取下一页的数据并且与当前页的数据合并</li></ul><h3 id="_4-1、改造课程模型类-实现上拉触底逻辑" tabindex="-1"><a class="header-anchor" href="#_4-1、改造课程模型类-实现上拉触底逻辑" aria-hidden="true">#</a> 4.1、改造课程模型类 - 实现上拉触底逻辑</h3><p>模型方法 <code>getCourseList(page,count,categoryId=null,type=null)</code> 中默认会接收 4 个参数，其中 categoryId 和 type 不是模型应该关心的部分，将 page 和 count 去掉，交给模型来维护（在模型类 Course 中定义 page 和 count 属性）</p><ul><li>将 <code>getCourseList</code> 方法中的 page 和 count 参数以类属性的方式进行初始化声明</li><li>将网络请求返回的结果 与 上一次的结果做数据合并</li><li>判断边界，如果当前页码 等于 最大页码（总记录数/每页数量），已经没有数据了，即不再发起数据请求</li><li>当前页码 等于 最大页码，即还有数据。当前页码就 +1 ，当下一次发起请求时 page 就会等于 2</li><li>最后，将最终获取到的数据返回到页面中即可</li></ul><p>在 <code>/model/course.js</code> 中</p><div class="language-javascript line-numbers-mode" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">import</span> Http <span class="token keyword">from</span> <span class="token string">&quot;../utils/http&quot;</span><span class="token punctuation">;</span>
...
<span class="token keyword">export</span> <span class="token keyword">default</span> Course<span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_5-2、实现下拉刷新" tabindex="-1"><a class="header-anchor" href="#_5-2、实现下拉刷新" aria-hidden="true">#</a> 5.2、实现下拉刷新</h3><p>直接调用原来实例化的对象，不需要再重新 new ，直接调用模型类中定义的 <code>reset()</code> 方法即可</p><p>在页面 <code>pages/index/index.js</code> 逻辑下拉刷新生命周期函数中</p><div class="language-javascript line-numbers-mode" data-ext="js"><pre class="language-javascript"><code><span class="token function">Page</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
  <span class="token comment">// 省略部分 .....</span>
  ...
  <span class="token punctuation">}</span><span class="token punctuation">,</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在控制台 network 中查看上拉触底、下拉刷新前后的参数变化，观察是否有初始化，同时观察页面数据是否有变化。</p><p><img src="https://www.arryblog.com/assets/img/image-20230428054351779.0cb9b921.png" alt="image-20230428054351779"></p><h3 id="_6、总结-面向对象在项目工程中的最佳实践" tabindex="-1"><a class="header-anchor" href="#_6、总结-面向对象在项目工程中的最佳实践" aria-hidden="true">#</a> 6、总结：面向对象在项目工程中的最佳实践</h3><p>什么时候定义实例方法（使用时需要 new），什么时候定义静态方法</p><ul><li>实例方法：当方法中需要使用到类的属性，并需要维护它们的状态时，就需要使用实例方法（实例对象，是有状态的）</li><li>静态方法：当方法的本身实现是不依赖类的属性（维护状态）时，就是用静态方法（在静态方法中，无法获取实例属性的值，这是静态方法的语法定义）</li></ul><p>如果我们做过后端开发就会知道，绝大多数的后端工具类库全部都是静态方法。都是纯粹的实现，不依赖任何状态，因此都是静态方法。</p><p>这种设计也很正常，通常情况下一个类中会存在状态（属性），多数情况会出现在业务模型或业务操作中，而作为第三方的公共工具类库是不可能跟具体的业务耦合的。</p><h3 id="_6-1、调用静态方法-和-实例化方法的本质总结" tabindex="-1"><a class="header-anchor" href="#_6-1、调用静态方法-和-实例化方法的本质总结" aria-hidden="true">#</a> 6.1、调用静态方法 和 实例化方法的本质总结</h3><ul><li>调用静态方法本质上就是 <strong>调用类方法</strong></li><li>实例化调用，本质上是在 <strong>调用对象的方法</strong></li></ul><blockquote><p>调用类方法 和 调用对象的方法 区别</p></blockquote><div class="language-javascript line-numbers-mode" data-ext="js"><pre class="language-javascript"><code><span class="token comment">// 实例化 Course 类</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><blockquote><p>这也是为什么 实例化调用，本质上是在 <strong>调用对象的方法</strong> 而不是 类方法</p></blockquote><h3 id="_6-2、静态属性" tabindex="-1"><a class="header-anchor" href="#_6-2、静态属性" aria-hidden="true">#</a> 6.2、静态属性</h3><p>我们知道 类里面除了有静态方法外，还有静态属性。通过之前的学习知道在静态方法中是无法访问到类里面定义的属性的。</p><blockquote><p>但，在静态方法中是可以访问静态属性的</p></blockquote><div class="language-javascript line-numbers-mode" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">Course</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span><span class="token punctuation">,</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_4、发现问题" tabindex="-1"><a class="header-anchor" href="#_4、发现问题" aria-hidden="true">#</a> 4、发现问题</h3><p>当我们在 tab 标签页之间频繁的切换，它的数据渲染就会出错，同时还会产生很多不必要的网络请求。</p><p>会出现我们可能已经切换到了“基础入门”，可能数据停留在 “正在学” 的数据。同样课程分类也一样。虽说这样无聊的用户会比较少一些，不停在两个标签之间来回频繁的切换。</p><blockquote><p>但类似这样问题在我们实际开发中经常会遇到，因此解决这个问题对我们来说非常有意义。</p></blockquote>`,131);function d(v,m){const t=e("ExternalLinkIcon");return o(),c("div",null,[u,s("p",null,[n("是否需要将课程列表信息封装为自定义组件，可根据 "),s("a",r,[n("自定义组件封装 - 最佳实践总结 (opens new window)"),l(t)]),n("中讲到《自定义组件封装的本质》满足条件即可对 课程列表部分进行封装。")]),k])}const g=p(i,[["render",d],["__file","JS mianxiangduixiang、fencengsheji、dongtaishujubianlizuijiashijianyuyingyong.html.vue"]]);export{g as default};
